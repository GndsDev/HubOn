...
@Component({
  ...
  styles: `
    ...
    .stock-row > strong:first-of-type,
    .movement-row > strong:nth-of-type(2) {
...
export class StockPageComponent implements OnInit {
  ...
  saveMovement(): void { const form = this.movementForm; if (!form.stockItemId || this.invalidMovementQuantity() || (this.requiresReason() && !form.reason.trim()) || this.saving()) return; this.saving.set(true); const reason = form.reason.trim() || null; const operation = form.type === 'ENTRY' ? this.api.entry({ stockItemId: form.stockItemId, quantity: Number(form.quantity), reason }) : form.type === 'EXIT' ? this.api.exit({ stockItemId: form.stockItemId, quantity: Number(form.quantity), reason }) : form.type === 'LOSS' ? this.api.loss({ stockItemId: form.stockItemId, quantity: Number(form.quantity), reason: form.reason.trim() }) : this.api.adjust({ stockItemId: form.stockItemId, newStock: Number(form.quantity), reason: form.reason.trim() }); operation.pipe(finalize(() => this.saving.set(false))).subscribe({ next: () => { this.movementDialog.set(false); this.feedback.success('Movimentação registrada.'); this.refreshStock(); }, error: (error) => this.feedback.error(apiErrorMessage(error)) }); }
  invalidMovementQuantity(): boolean { return this.movementForm.type === 'ADJUSTMENT' ? this.movementForm.quantity < 0 : this.movementForm.quantity <= 0; }
